...
const uri =
  "mongodb+srv://chandu:chandu@cluster0.y9hnpwu.mongodb.net/Images?retryWrites=true&w=majority";
const uri2 =
  "mongodb+srv://chandu:chandu@cluster0.y9hnpwu.mongodb.net/?retryWrites=true&w=majority";
...
async function createRestaurant(client, restaurant) {
  ...
  console.log(
    `New restaurant registered with the following id: ${result.insertedId}`
  );
}
...
async function createOrganization(client, organization) {
  ...
  console.log(
    `New organization registered with the following id: ${result.insertedId}`
  );
}
...
async function addFoodItem(client, email, foodItem) {
  ...
  console.log(
    `New food item  added with the following id: ${result.insertedId}`
  );
}
...
async function addFoodRecipe(client, foodRecipe) {
  const result = await client
    ...
    .insertOne(foodRecipe);
  console.log(
    `New food Recipe added with the following id: ${result.insertedId}`
  );
}
...
async function addAnnouncement(client, announcement) {
  ...
  console.log(
    `New announcement added with the following id: ${result.insertedId}`
  );
}
...
async function addUserFeedback(client, userid, feedback) {
  const result = await client
    ...
    .insertOne({ customerID: userid, feedback: feedback });
  console.log(
    `New Feedback Registered with the following id: ${result.insertedId}`
  );
}
...
async function getDonationsByStatusAndOrganization(
  client,
  organization,
  status
) {
  const cursor = await client
    ...
    .collection("donations")
    .find({
      $and: [{ status: status }, { organization: organization }],
    });
  const results = await cursor.toArray();
  return results;
}
...
async function getDonationsByStatusAndRestaurant(client, restaurant, status) {
  const cursor = await client
    ...
    .collection("donations")
    .find({
      $and: [{ status: status }, { restaurant: restaurant }],
    });
  const results = await cursor.toArray();
  return results;
}
...
async function updateUserName(client, id, value) {
  const result = await client
    .db("hungrezy")
    .collection("users")
    .updateOne({ mobileNumber: id }, { $set: { name: value } });
}
...
async function updateUserEmail(client, id, value) {
  const result = await client
    .db("hungrezy")
    .collection("users")
    .updateOne({ mobileNumber: id }, { $set: { email: value } });
}
...
async function updateUserGender(client, id, value) {
  const result = await client
    .db("hungrezy")
    .collection("users")
    .updateOne({ mobileNumber: id }, { $set: { gender: value } });
}
...
async function updateUserPassword(client, id, value) {
  const result = await client
    .db("hungrezy")
    .collection("users")
    .updateOne({ mobileNumber: id }, { $set: { password: value } });
}
...
async function updateOrderStatus(client, email, ID) {
  const result = await client
    ...
    .updateOne(
      { orderID: ID },
      { $set: { orderStatus: "Delivery Completed" } }
    );
}
...
app.get("/Restaurants", async function (req, res) {
...

  {
    try {
      ...
      res.render("pages/Explore_Restuarants", {
        foodItems: fooditems,
...
app.get("/helpAndSupport", async function (req, res) {
  ...
  res.render("pages/helpAndSupport", {
    pageTitle: pageTitle,
    currentUser: currentUser,
    announcements,
  });
...
app.post("/updateUserProfile", async function (req, res) {
  ...
  if (currentUser) {
    if (name != "" && name.length > 0 && name != currentUser.name)
      await updateUserName(client, currentUser.mobileNumber, name);
    if (email != "" && email.length > 0 && email != currentUser.email)
      await updateUserEmail(client, currentUser.mobileNumber, email);
    if (gender != "" && gender.length > 0 && gender != currentUser.gender)
      await updateUserGender(client, currentUser.mobileNumber, gender);
...
app.get("/Menu", async function (req, res) {
    ...
    MenuItems.map((item) => {
      if (name == item.category) {
        ...
        foodItemImages.map((image) => {
          if (image._id == item.name) foodItemImage = image;
        });
        array.push({ item: item, image: foodItemImage });
        if (name == "Recomended")
          recomended.push({ item: item, image: foodItemImage });
      }
    });
    if (name != "Recomended" && array.length > 0)
      Menu.push({ categoryName: name, items: array });
  });
  ...
  const menuItems = JSON.stringify(MenuItems)
    .replace(/\\/g, "\\\\")
    .replace(/"/g, '\\"');
  res.render("pages/Restuarant_Menu", {
        ...
        userOrders.map((order) => {
          restaurants.map((restaurant) => {
            if (order.restaurantID == restaurant._id)
              Orders.push({ order: order, restaurant: restaurant });
          });
        });
...
app.get("/Admin", async function (req, res) {
  ...
  const approvedRestaurants = await getRestaurantsByStatus(client, "approved");
  const suspendedRestaurants = await getRestaurantsByStatus(
    client,
    "suspended"
  );
  const pendingOrganizations = await getOrganizationsByStatus(
    client,
    "pending"
  );
  const approvedOrganizations = await getOrganizationsByStatus(
    client,
    "approved"
  );
  const suspendedOrganizations = await getOrganizationsByStatus(
    client,
    "suspended"
  );
  ...
  if (req.cookies.email == null) {
    ...
    res.render("pages/Admin", {
      ...
      pageTitle: pageTitle,
      pendingRestaurantVerifications: pendingRestaurants,
...
app.get("/Add_Recipe", async function (req, res) {
  ...
  if (req.cookies.email != null) {
    ...
    res.render("pages/Admin_AddRecipe", {
      currentUser: currentAdmin,
      pageTitle: pageTitle,
    });
  } else {
...
app.get("/Restaurants_Home", async function (req, res) {
  ...
  if (req.cookies.restaurantEmail != null) {
    currentRestaurant = await getRestaurantByEmail(
      client,
      req.cookies.restaurantEmail
    );
    getUsers(client).then((users) => {
      getRestaurantOrders(client, currentRestaurant._id).then(
        (restaurantOrders) => {
          restaurantOrders.map((order) => {
            users.map((user) => {
              if (order.customerID == user._id)
                Orders.push({ order: order, user: user });
            });
          });
          res.render("pages/Restaurants_Home", {
            pageTitle: pageTitle,
...
app.get("/Organizations_Home", async function (req, res) {
  ...
  if (req.cookies.organizationEmail != null) {
    currentOrganization = await getOrganizationByEmail(
      client,
      req.cookies.organizationEmail
    );
    const pendingDonations = await getDonationsByStatusAndOrganization(
      client,
      currentOrganization.name,
      "pending"
    );
    const rejectedDonations = await getDonationsByStatusAndOrganization(
      client,
      currentOrganization.name,
      "rejected"
    );
    const acceptedDonations = await getDonationsByStatusAndOrganization(
      client,
      currentOrganization.name,
      "accepted"
    );
    const sendingDonations = await getDonationsByStatusAndOrganization(
      client,
      currentOrganization.name,
      "sending"
    );
    const successDonations = await getDonationsByStatusAndOrganization(
      client,
      currentOrganization.name,
      "success"
    );
    const notSuccessDonations = await getDonationsByStatusAndOrganization(
      client,
      currentOrganization.name,
      "not success"
    );
    res.render("pages/Organizations_Home", {
      pageTitle: pageTitle,
      currentUser: currentOrganization,
      donations: pendingDonations,
      sendingDonations: sendingDonations,
      successDonations: successDonations,
      rejectedDonations: rejectedDonations,
      acceptedDonations: acceptedDonations,
      notSuccessDonations: notSuccessDonations,
    });
  } else {
    res.redirect("/Organization_Login");
  }
...
app.get("/Donations", async function (req, res) {
  ...
  if (req.cookies.restaurantEmail != null) {
    ...
    const pendingDonations = await getDonationsByStatusAndRestaurant(
      client,
      currentRestaurant.name,
      "pending"
    );
    const rejectedDonations = await getDonationsByStatusAndRestaurant(
      client,
      currentRestaurant.name,
      "rejected"
    );
    const acceptedDonations = await getDonationsByStatusAndRestaurant(
      client,
      currentRestaurant.name,
      "accepted"
    );
    const sendingDonations = await getDonationsByStatusAndRestaurant(
      client,
      currentRestaurant.name,
      "sending"
    );
    const successDonations = await getDonationsByStatusAndRestaurant(
      client,
      currentRestaurant.name,
      "success"
    );
    const notSuccessDonations = await getDonationsByStatusAndRestaurant(
      client,
      currentRestaurant.name,
      "not success"
    );
...
app.get("/Donate", async function (req, res) {
  ...
  if (req.cookies.restaurantEmail != null) {
    ...
    res.render("pages/Donate", {
      pageTitle: pageTitle,
      currentUser: currentRestaurant,
      Organization: approvedOrganizations,
      MenuItems: MenuItems,
    });
  } else {
    res.redirect("/Restaurant_Login");
